import { FDCSectionHelper } from '../../../../SAPAssetManager/Rules/FDC/DynamicPageGenerator';
...
import { SetStartPointEndPointLength } from '../../../../SAPAssetManager/Rules/LAM/CreateUpdate/LAMCreateUpdateCalculateLengthStartEndPoint';
import prevReadingQuery from '../../../../SAPAssetManager/Rules/Measurements/Points/MeasurementDocumentPreviousReadingQuery';
    ...
    return fdcHelper.run(async (sectionBinding, section) => {
        ...
        if (skip) {
            ...
            if (sectionBinding['@odata.type'] === '#sap_mobile.MeasuringPoint') {
                previousReadingPromise = MeasuringPointLibrary.getLatestNonLocalReading(context, sectionBinding, prevReadingQuery(context, sectionBinding));
            } else {
                // If MeasuringPoint is undefined, assume we're on a PRT Point
                if (!sectionBinding.MeasuringPoint) {
                    previousReadingPromise = MeasuringPointLibrary.getLatestNonLocalReading(context, sectionBinding.PRTPoint, prevReadingQuery(context, sectionBinding));
                } else {
                    previousReadingPromise = MeasuringPointLibrary.getLatestNonLocalReading(context, sectionBinding.MeasuringPoint, prevReadingQuery(context, sectionBinding));
                }
            }
